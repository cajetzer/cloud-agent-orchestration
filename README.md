# GitHub Agentic Workflows — Multi-Agent Orchestration

A **learning and demo repository** showing how to use **GitHub Agentic Workflows** (technical preview) for multi-agent orchestration of multiple custom agents defined in agent.md files.
...
```mermaid
flowchart TD
    Issue["🎫 GitHub Issue\nCreate a pipeline to copy\ndata from Blob to SQL Database\nLabel: adf-pipeline"]
    Issue -->|"Issue labeled\ntriggers orchestrator"| Orch

    subgraph Orch["🎯 ADF ORCHESTRATOR — adf-orchestrator.md"]
        O1["1. Validate issue has requirements"]
        O2["2. Dispatch generation worker"]
        O3["3. Monitor for PR creation"]
        O4["4. Dispatch review worker"]
        O5["5. Handle review results"]
        O1 --> O2 --> O3 --> O4 --> O5
    end

    Orch -->|dispatch-workflow| Gen
    Orch -->|dispatch-workflow| Rev

    subgraph Gen["⚙️ GENERATION WORKER — adf-generate-worker.md"]
        GT["Tools: github, edit, bash"]
        GC["Capabilities:\n• Read templates\n• Generate JSON\n• Create PR"]
    end

    subgraph Rev["🔍 REVIEW WORKER — adf-review-worker.md"]
        RT["Tools: github,\nadf-knowledge-base MCP, bash"]
        RC["Capabilities:\n• Query common issues DB\n• Check anti-patterns\n• Get fix resolutions\n• Post detailed review"]
    end

    Gen -->|"safe-output:\ncreate-pull-request"| PR
    Rev -->|"safe-output:\nadd-comment, add-labels"| PR

    PR["📋 Pull Request\n• Pipeline JSON generated by worker\n• Review findings with KB references\n• Labels: approved / changes-requested /\napproved-with-warnings"]
```
